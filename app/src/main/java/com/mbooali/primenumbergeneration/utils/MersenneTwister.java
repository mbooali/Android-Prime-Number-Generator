...
public strictfp class MersenneTwister extends java.util.Random implements Serializable, Cloneable {
    ...
    public static BigInteger GenerateForSeed(long SEED, int num_of_bits) {
        int j;
        ...
        long nextSeed = 0;
        BigInteger nextBig = BigInteger.ZERO;
        for (j = 0; ret.bitLength() < num_of_bits; j++) {
            nextSeed = r.nextLong();
            nextBig = BigInteger.valueOf(nextSeed);
            ret = ret.shiftLeft(nextBig.bitLength()).add(nextBig);
            r.setSeed(nextSeed);
        ...
        ret = ret.shiftRight(ret.bitLength() - num_of_bits);
        ret = ret.negate();
    ...
    public static void main(String args[]) {
        ...
        BigInteger p = GenerateForSeed(SEED, 2048);
